...
def get_char_info(data: [dict]):
    '''Returns a dictionary of the number of female and male characters filled for the given movies in the csv file'''
    results = data[0]
...
def get_meta_data(data: [dict]):
    '''Adds the imbid, year, gross to the dictionary based on the given movies in the csv file'''
    results = data[0]
    final_results = data[1]
    with open('meta_data7.csv', mode = 'r') as csv_file:
        csv_reader = csv.DictReader(csv_file)
